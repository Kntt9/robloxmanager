...
impl RobloxClient {
    ...
    async fn request_inner(
        ...
        self.request_inner_with_ua(method, url, cookie, body, None)
            .await
    }

    async fn request_inner_with_ua(
        &self,
        method: Method,
        url: &str,
        cookie: &str,
        ...
        loop {
            ...
            // A per-request User-Agent override (WEAO / exploits API). The
            // client's default UA stays for everything Roblox-facing.
    ...
    /// Like [`get_json`] but with a custom `User-Agent` header, used by the
    /// WEAO / exploits API which requires `WEAO-3PService`.
    pub async fn get_json_ua<T: DeserializeOwned>(
        &self,
        url: &str,
        cookie: &str,
        user_agent: &str,
        ...
        let resp = self
            .request_inner_with_ua(Method::GET, url, cookie, ReqBody::None, Some(user_agent))
            .await?;
        let status = resp.status();
        if !status.is_success() {
            let msg = resp.text().await.unwrap_or_default();
