...
pub enum BackendCommand {
    ...
    ArrangeWindows,
    /// Fetch the currently most-played Roblox games. Unauthenticated.
    FetchPopularGames { sort: ram_core::api::GameSort, limit: usize },
    /// Search Roblox for games by name. Unauthenticated.
...
pub enum BackendEvent {
    ...
    WindowsArranged,
    /// A popular-games feed was fetched.
    PopularGamesFetched {
        sort: ram_core::api::GameSort,
        games: Vec<ram_core::models::PopularGame>,
        error: Option<String>,
    },
    /// A game search completed.
    GamesSearchResult {
        games: Vec<ram_core::models::PopularGame>,
        error: Option<String>,
    },
    /// Thumbnail PNG bytes for the Games tab, keyed by universe ID. Downloaded
    /// on the backend because egui's HTTP loader fails against the Roblox CDN.
    GameThumbnailsReady(Vec<(u64, Vec<u8>)>),
    /// A single place ID was resolved to a game preview for the Launch panel.
    GamePreviewResolved {
        place_id: u64,
        /// Game name, or empty if resolution failed.
        name: String,
        /// Download PNG bytes of the game icon, or empty if unavailable.
        thumb_bytes: Vec<u8>,
...
async fn handle_command(
    ...
    match cmd {
        ...
        BackendCommand::FetchPopularGames { sort, limit } => {
            match api::fetch_popular_games(client, sort, limit).await {
                Ok(games) => {
                    // Deliver thumbnails as bytes over the event channel (egui's
                    // HTTP loader cannot reach Roblox's CDN from this build).
                    let thumbs = api::download_game_thumbnails(client, &games).await;
                    if !thumbs.is_empty() {
                        let _ = tx.send(BackendEvent::GameThumbnailsReady(thumbs));
                    }
                    Ok(BackendEvent::PopularGamesFetched {
                        sort,
                        games,
                        error: None,
                    })
                ...
                Err(e) => {
                    // Reported as data rather than `Err`: a failed feed is
                    // normal (offline, Roblox changed the endpoint again) and
                    // the games tab should show an error state, not spam toasts.
                    info!("Popular games fetch failed (non-fatal): {e}");
                    Ok(BackendEvent::PopularGamesFetched {
                        sort,
                        games: Vec::new(),
        ...
        BackendCommand::SearchGames { query, limit } => {
            match api::search_games(client, &query, limit).await {
                Ok(games) => {
                    let thumbs = api::download_game_thumbnails(client, &games).await;
                    if !thumbs.is_empty() {
                        let _ = tx.send(BackendEvent::GameThumbnailsReady(thumbs));
                    }
                    Ok(BackendEvent::GamesSearchResult {
                        games,
                        error: None,
                    })
                }
                Err(e) => {
                    info!("Game search failed (non-fatal): {e}");
                    Ok(BackendEvent::GamesSearchResult {
                        games: Vec::new(),
                        error: Some(e.to_string()),
                    })
        ...
        BackendCommand::FetchExploits => {
            match ram_core::exploits_api::fetch_exploits(client).await {
                Ok(exploits) => Ok(BackendEvent::ExploitsFetched {
                    exploits,
                    error: None,
                }),
                Err(e) => {
                    info!("Exploits fetch failed (non-fatal): {e}");
                    Ok(BackendEvent::ExploitsFetched {
                        exploits: Vec::new(),
                        error: Some(e.to_string()),
                    })
                }
        ...
        BackendCommand::ResolveGamePreview { place_id } => {
            // Bound the whole resolution so the UI can never be stuck on
            // "Identifying game…": after this window, we return an error.
            let deadline = tokio::time::sleep(std::time::Duration::from_secs(10));
            tokio::pin!(deadline);
            let outcome = tokio::select! {
                r = api::resolve_game_preview(client, place_id) => r,
                _ = &mut deadline => Err(ram_core::CoreError::RobloxApi {
                    status: 0,
                    message: format!("timed out resolving place {place_id}"),
                }),
            };
            match outcome {
                Ok(preview) => {
                    // Download the icon bytes here so egui's HTTP loader (which
                    // cannot reach the Roblox CDN) never has to.
                    let thumb_bytes = if preview.thumb_url.is_empty() {
                        Vec::new()
                    } else {
                        client
                            .get_bytes(&preview.thumb_url, "")
                            .await
                            .unwrap_or_default()
                    };
                    Ok(BackendEvent::GamePreviewResolved {
                        place_id,
                        name: preview.name,
                        thumb_bytes,
                        error: None,
                ...
                Err(e) => Ok(BackendEvent::GamePreviewResolved {
                    place_id,
                    name: String::new(),
                    thumb_bytes: Vec::new(),
                    error: Some(e.to_string()),
