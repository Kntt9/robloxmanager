...
/// Actions from within the trash panel.
enum TrashAction {
    /// Restore the trashed account at `store.trashed[idx]`.
    Restore(usize),
    /// Prompt to permanently delete the trashed account at `idx`.
    PermanentDelete(usize),
    /// Prompt to permanently delete every trashed account.
    EmptyTrash,
}
...
pub struct AppState {
    ...
    roblox_instance_count: usize,
    /// Cached "most played games" feeds, keyed by sort. Refreshed on demand.
    popular_games: HashMap<ram_core::api::GameSort, Vec<ram_core::models::PopularGame>>,
    /// Per-sort fetch errors, so each tab can show its own error state.
    popular_games_error: HashMap<ram_core::api::GameSort, String>,
    /// Which sorts have been requested at least once, so opening the tab
    /// triggers exactly one automatic fetch per sort rather than one per frame.
    popular_games_requested: HashSet<ram_core::api::GameSort>,
    /// Latest game-search results, if a search is active. `None` = not
    /// searching (show the selected sort feed instead).
    games_search: Option<Vec<ram_core::models::PopularGame>>,
    /// Search error, if the last search failed.
    games_search_error: Option<String>,
    /// Search text buffer for the Games tab.
    games_search_input: String,
    /// Which Games tab sort is selected.
    games_state: games::GamesState,
    /// Cached WEAO exploit status feed, refreshed on demand.
    exploits: Vec<ram_core::exploits_api::ExploitInfo>,
    /// Set when the last exploit fetch failed, so the tab can show the error.
    exploits_error: Option<String>,
    /// Whether the tab has asked for a fetch at least once.
    exploits_requested: bool,
    /// Whether a fetch is currently in flight (to show a loading state).
    exploits_loading: bool,
    /// State for the exploits panel (animations, last fetch time).
    exploits_state: exploits::ExploitsState,
    /// Whether the disclaimer modal is currently open.
    exploits_disclaimer_open: bool,
    /// Whether the user has checked the "I understand" checkbox.
    exploits_disclaimer_checked: bool,
    /// Downloaded game thumbnail bytes for the Games tab, keyed by universe ID.
    game_thumbnails: HashMap<u64, Vec<u8>>,
    /// Whether the presets management window is open (launched from Accounts).
    presets_open: bool,
    /// Resolved game previews (name + icon URL) for the Launch panel, keyed by
    /// place ID. Populated on demand with a debounce; see
    /// [`Self::maybe_resolve_game_preview`].
    game_preview_cache: HashMap<u64, ram_core::api::GamePreview>,
    /// Downloaded preview thumbnail bytes keyed by place ID.
    game_preview_thumbs: HashMap<u64, Vec<u8>>,
    /// Place IDs currently being resolved, so we don't double-request.
    game_preview_inflight: HashSet<u64>,
    /// Debounce bookkeeping for the preview: last seen place ID value and the
    /// earliest instant we may issue the next resolve. Prevents a request per
    /// keystroke while typing a Place ID.
    game_preview_debounce: Option<(Option<u64>, std::time::Instant)>,
    ...
    /// Whether the trash panel window is open.
    trash_open: bool,
    /// Index into `store.trashed` pending permanent-deletion confirmation.
...
impl AppState {
    pub fn new(mut config: AppConfig, config_path: PathBuf) -> Self {
        ...
        let mut state = Self {
            ...
            popular_games: HashMap::new(),
            popular_games_error: HashMap::new(),
            popular_games_requested: HashSet::new(),
            games_search: None,
            games_search_error: None,
            games_search_input: String::new(),
            games_state: games::GamesState::default(),
            game_thumbnails: HashMap::new(),
            presets_open: false,
            exploits: Vec::new(),
            exploits_error: None,
            exploits_requested: false,
            exploits_loading: false,
            exploits_state: exploits::ExploitsState::default(),
            exploits_disclaimer_open: false,
            exploits_disclaimer_checked: false,
            game_preview_cache: HashMap::new(),
            game_preview_thumbs: HashMap::new(),
            game_preview_inflight: HashSet::new(),
            game_preview_debounce: None,
    ...
    fn process_events(&mut self) {
        for event in self.bridge.poll() {
            match event {
                ...
                BackendEvent::PopularGamesFetched { sort, games, error } => {
                    self.popular_games.insert(sort, games);
                    match error {
                        Some(err) => {
                            self.popular_games_error.insert(sort, err);
                        }
                        None => {
                            self.popular_games_error.remove(&sort);
                        }
                    }
                }
                BackendEvent::GamesSearchResult { games, error } => {
                    self.games_search = Some(games);
                    self.games_search_error = error;
                }
                BackendEvent::GameThumbnailsReady(thumbs) => {
                    for (uid, bytes) in thumbs {
                        self.game_thumbnails.insert(uid, bytes);
                    }
                }
                BackendEvent::ExploitsFetched { exploits, error } => {
                    self.exploits = exploits;
                    self.exploits_error = error;
                ...
                BackendEvent::GamePreviewResolved {
                    ...
                    self.game_preview_inflight.remove(&place_id);
                    if error.is_some() {
                        // Cache a sentinel so we don't retry every frame for an
                        // invalid/unknown place ID; the UI shows a "not found"
                        // state instead.
                        self.game_preview_cache.insert(
                            place_id,
                            ram_core::api::GamePreview {
                                place_id,
                                ..Default::default()
                            },
                        );
                        self.game_preview_thumbs.remove(&place_id);
                    } else {
                        self.game_preview_cache.insert(
                            place_id,
                            ram_core::api::GamePreview {
    ...
    /// Export all accounts to a portable file. Cookies are decrypted on disk,
    /// so the file must be treated like a password manager vault backup.
    fn export_accounts(&mut self) {
        ...
        if self.store.accounts.is_empty() {
            self.toasts.push(Toast::info("No accounts to export"));
            return;
        }
        ...
        // Confirm, because the file will contain live sessions.
        let ok = rfd::MessageDialog::new()
            .set_level(rfd::MessageLevel::Warning)
            .set_title("Export contains your logins")
            .set_description(
                "This file will contain the plaintext login cookies of every account.\n\n\
                 Anyone who gets it can sign in as them. Keep it somewhere safe, \
                 and delete it after you no longer need it.\n\n\
                 Continue?",
            )
        ...
        match ram_core::transfer::export_store(&self.store, &session, &path) {
            Ok(n) => {
                self.toasts.push(Toast::success(format!(
                    "Exported {n} account(s) to {}",
                    path.display()
                )));
                tracing::info!("Exported {n} account(s) to {}", path.display());
            }
            Err(e) => {
                self.toasts
    ...
    /// Import accounts from an export file. Cookies are re-encrypted under the
    /// current store session; existing accounts (same user ID) are left as they
    /// are rather than overwritten.
    fn import_accounts(&mut self) {
        ...
        let imported = match ram_core::transfer::import_store(&path, &session) {
            Ok(accounts) => accounts,
            Err(e) => {
                self.toasts
                    .push(Toast::error(format!("Import failed: {e}")));
                return;
            }
        };
        if imported.is_empty() {
            self.toasts.push(Toast::info("The file contains no accounts"));
            return;
        }
        let mut added = 0usize;
        let mut skipped = 0usize;
        for account in imported {
            if self.store.find_by_id(account.user_id).is_some() {
                skipped += 1;
                continue;
            }
            self.store.accounts.push(account);
        ...
        if added > 0 {
            self.auto_save();
            self.toasts.push(Toast::success(format!(
                "Imported {added} account(s) ({skipped} already present, skipped)"
            )));
            tracing::info!("Imported {added} account(s), skipped {skipped} existing");
        } else {
            self.toasts.push(Toast::info(format!(
                "All {skipped} account(s) from the file are already in RM"
            )));
        }
    }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.selectable_value(&mut self.active_tab, Tab::Statistics, format!("📊 {}", t("Stats")));
                ui.selectable_value(&mut self.active_tab, Tab::Games, format!("🎮 {}", t("Games")));
                ui.selectable_value(&mut self.active_tab, Tab::Exploits, format!("\u{2699} {}", t("Exploits")));
                ui.selectable_value(&mut self.active_tab, Tab::Settings, format!("⚙ {}", t("Settings")));
        ...
        match self.active_tab {
            ...
            Tab::Exploits => self.show_exploits_tab(ctx),
        }

        // ---- Floating add-account dialog ----
        self.show_add_dialog(ctx);

        // ---- Exploits disclaimer (locked until accepted) ----
        self.show_exploits_disclaimer(ctx);

        // ---- Presets manager (opened from the Accounts tab) ----
        self.show_presets_window(ctx);

        // ---- Confirmation dialog for account removal (moves to trash) ----
...
impl AppState {
    fn show_accounts_tab(&mut self, ctx: &egui::Context) {
        // Resolve the game preview for the current Place ID (debounced).
        self.maybe_resolve_game_preview();
            ...
            .show(ctx, |ui| {
                ...
                for a in result.actions {
                    match a {
                        ...
                        sidebar::SidebarAction::ShowPresets => {
                            self.presets_open = true;
                        }
                        sidebar::SidebarAction::OpenTrash => {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            if self.selected_ids.len() > 1 {
                ...
                if let Some(account) = account {
                    ...
                    let preset_view: Vec<ram_core::models::LaunchPreset> =
                        self.presets.iter().map(|(_, p)| p.clone()).collect();
                    // Game preview for the current Place ID, if resolved.
                    let preview_place = self
                        .main_panel_state
                        .place_id_input
                        .trim()
                        .parse::<u64>()
                        .ok();
                    let preview = preview_place.and_then(|p| {
                        self.game_preview_cache.get(&p).cloned()
                    });
                    let preview_thumbs = preview_place
                        .and_then(|p| self.game_preview_thumbs.get(&p).cloned());
                    ...
                    if let Some(a) = result.action {
                        match a {
                            ...
                            main_panel::MainPanelAction::UpdateNotes { user_id, notes } => {
                                if let Some(acc) = self.store.find_by_id_mut(user_id) {
                                    acc.notes = notes;
                                }
                                self.auto_save();
                            }
                            main_panel::MainPanelAction::SaveLaunchTarget { user_id } => {
                                if let Some(acc) = self.store.find_by_id_mut(user_id) {
                                    acc.saved_place_id =
                                        self.main_panel_state.place_id_input.clone();
                                    acc.saved_job_id =
                                        self.main_panel_state.job_id_input.clone();
                                }
                                self.auto_save();
                            }
    ...
    fn show_presets_window(&mut self, ctx: &egui::Context) {
        ...
        // Handle the requested action outside the window closure so we can
        // mutate other parts of self without conflicting borrows.
        ...
        match action {
            ...
            presets_panel::PresetsAction::Rescan => {
                self.reload_presets();
            }
        }
    }

    fn show_stats_tab(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            stats::show(
                ui,
                &self.store,
    ...
    fn show_games_tab(&mut self, ctx: &egui::Context) {
        // Fetch each sort on first visit, and only then. Re-fetching is the
        // Refresh button's job, so this does not double up on every frame.
        for sort in games::ALL_SORTS {
            if !self.popular_games_requested.contains(&sort) {
                self.popular_games_requested.insert(sort);
                self.fetch_popular_games(sort);
            }
        }
        let mut pending: Option<games::GamesAction> = None;
        egui::CentralPanel::default().show(ctx, |ui| {
            pending = games::show(
                ui,
                &mut self.games_state,
                &self.popular_games,
                &self.popular_games_error,
                &self.game_thumbnails,
                &self.games_search,
                self.games_search_error.as_deref(),
                self.games_search_input.as_str(),
                &self.config.favorite_games,
            );
        });
        let Some(action) = pending else { return };
        match action {
            games::GamesAction::Refresh => {
                for sort in games::ALL_SORTS {
                    self.fetch_popular_games(sort);
                }
            }
            games::GamesAction::SetSort(sort) => {
                // Selecting a sort with no data yet fetches it; otherwise it is
                // already in the cache.
                if !self.popular_games.contains_key(&sort) {
                    self.fetch_popular_games(sort);
                }
            }
            games::GamesAction::Search(query) => {
                self.games_search_input = query.clone();
                if query.is_empty() {
                    // Clear search results when the user clears the search box.
                    self.games_search = None;
                } else {
                    self.bridge
                        .send(BackendCommand::SearchGames { query, limit: 30 });
                }
            }
            games::GamesAction::CopyPlaceId(place_id) => {
                ctx.output_mut(|o| o.copied_text = place_id.to_string());
                self.toasts.push(Toast::success(format!(
                    "Copied Place ID {place_id}"
                )));
            }
            games::GamesAction::ToggleFavorite(universe_id) => {
                let favs = &mut self.config.favorite_games;
                if let Some(pos) = favs.iter().position(|&u| u == universe_id) {
                    favs.remove(pos);
                } else {
                    favs.push(universe_id);
                }
                let _ = self.config.save(&self.config_path);
            }
        }
    ...
    /// Kick off a fetch of one popular-games feed on the backend thread.
    fn fetch_popular_games(&self, sort: ram_core::api::GameSort) {
        self.bridge
            .send(BackendCommand::FetchPopularGames { sort, limit: 50 });
    }

    /// Fetch the WEAO exploit status feed. No-op while one is in flight.
    fn fetch_exploits(&mut self) {
        if self.exploits_loading {
            return;
        }
        self.exploits_loading = true;
        self.exploits_requested = true;
        self.bridge.send(BackendCommand::FetchExploits);
    }

    fn show_exploits_tab(&mut self, ctx: &egui::Context) {
        // If the disclaimer has not been accepted yet, open the modal
        // automatically so the user sees it right away — no card, no extra
        // click. The modal is drawn over the whole app, not inside this tab.
        if !self.config.exploits_disclaimer_accepted {
            self.exploits_disclaimer_open = true;
            // Don't fetch yet — that happens after the user accepts.
        } else if !self.exploits_requested {
            self.fetch_exploits();
        }

        let mut pending: Option<exploits::ExploitsAction> = None;
        egui::CentralPanel::default().show(ctx, |ui| {
            pending = exploits::show(
                ui,
                &mut self.exploits_state,
                &self.exploits,
                self.exploits_error.as_deref(),
                self.exploits_loading,
                self.config.exploits_disclaimer_accepted,
            );
        });
        let Some(action) = pending else { return };
        match action {
            exploits::ExploitsAction::Refresh => self.fetch_exploits(),
            exploits::ExploitsAction::OpenUrl(url) => {
                ctx.output_mut(|o| {
                    o.open_url = Some(egui::output::OpenUrl::new_tab(url));
                });
            }
            exploits::ExploitsAction::ReOpenDisclaimer => {
                self.exploits_disclaimer_open = true;
                self.exploits_disclaimer_checked = false;
            }
        }
    }

    /// The Exploits disclaimer modal — a centered overlay with a dark backdrop
    /// that blocks the rest of the app until the user explicitly accepts.
    ///
    /// * "Voltar" closes the modal without recording anything; re-entering the
    ///   Exploits tab re-opens it (the acceptance is never set by Voltar).
    /// * "Concordar" is disabled until the checkbox is ticked, and only that
    ///   button persists the acceptance.
    ///
    /// Once accepted, opening the tab again does not show the modal. A discrete
    /// "Ver termos" button in the tab re-opens it purely for review, and
    /// closing that copy does not revoke the stored acceptance.
    fn show_exploits_disclaimer(&mut self, ctx: &egui::Context) {
        if !self.exploits_disclaimer_open {
            return;
        }
        let lang = crate::i18n::of(ctx);
        ...
        egui::Modal::new(egui::Id::new("exploits_disclaimer_modal"))
            .frame(frame)
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.heading(
                        egui::RichText::new(t("Warning - Exploits"))
                            .size(20.0)
                            .strong(),
                    );
                });
                ...
                ui.label(t("Exploits disclaimer body"));

                ui.add_space(16.0);
                ui.separator();
                ui.add_space(8.0);
                ui.checkbox(
                    &mut self.exploits_disclaimer_checked,
                    t("I understand the risks and want to access the Exploits area."),
                );
                ui.add_space(14.0);

                // Buttons, right-aligned.
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let can_continue = self.exploits_disclaimer_checked;
                    if ui
                        .add_enabled(
                            can_continue,
                            egui::Button::new(
                                egui::RichText::new(t("Agree"))
                                    .strong()
                                    .color(ui.theme().on_accent),
                            )
                        ...
                        .clicked()
                    {
                        self.config.exploits_disclaimer_accepted = true;
                        self.exploits_disclaimer_checked = false;
                        let _ = self.config.save(&self.config_path);
                        should_close = true;
                    }
                    ...
                    if ui
                        .add(
                            egui::Button::new(egui::RichText::new(t("Back")))
                                .min_size(egui::vec2(90.0, 34.0)),
                        )
                        .clicked()
                    {
                        // Voltar = no acceptance recorded. The modal closes and
                        // the tab stays blocked; entering again re-opens it.
                        should_close = true;
                    }
                });
            });
        ...
        if should_close {
            self.exploits_disclaimer_open = false;
            if self.config.exploits_disclaimer_accepted && !self.exploits_requested {
                // Newly accepted: unlock and load the feed.
                self.fetch_exploits();
            } else if !self.config.exploits_disclaimer_accepted {
                // User backed out without accepting — leave the Exploits tab.
                self.active_tab = Tab::Accounts;
            }
        }
    }

    /// Debounce window for resolving the game preview from the Place ID field.
    const GAME_PREVIEW_DEBOUNCE_MS: u64 = 400;

    /// Resolve the game preview for the current Place ID input, debounced, and
    /// only once per place (cached + in-flight guards).
    ///
    /// Called every frame; the guards make it cheap. The current Place ID is
    /// the shared launch input owned by `main_panel_state`, which is the same
    /// field used by both single and bulk launch, so the preview always tracks
    /// what the user is about to launch.
    ///
    /// The debounce must *persist* the deadline across frames. The naive
    /// "recompute `now + debounce` each call and bail if we are early" never
    /// fires, because a fresh deadline is minted on every frame and `now` is
    /// always earlier than it.
    fn maybe_resolve_game_preview(&mut self) {
        let raw = self.main_panel_state.place_id_input.trim();
        let Ok(place_id) = raw.parse::<u64>() else {
            self.game_preview_debounce = Some((None, std::time::Instant::now()));
            return;
        };
        // Already resolved (or known to be invalid).
        if self.game_preview_cache.contains_key(&place_id) {
            self.game_preview_debounce = Some((Some(place_id), std::time::Instant::now()));
            return;
        }
        if self.game_preview_inflight.contains(&place_id) {
            return;
        }
        let now = std::time::Instant::now();
        // If the value has been stable since the last frame, keep the *same*
        // deadline so it is not pushed forward every frame. Only a changed
        // value restarts the window.
        let due = match self.game_preview_debounce {
            Some((Some(last), at)) if last == place_id => at,
            _ => now + std::time::Duration::from_millis(Self::GAME_PREVIEW_DEBOUNCE_MS),
        };
        if now < due {
            self.game_preview_debounce = Some((Some(place_id), due));
            return;
        }
        self.game_preview_inflight.insert(place_id);
        self.game_preview_debounce = Some((Some(place_id), now));
    ...
    fn show_settings_tab(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            match action {
                ...
                Some(settings::SettingsAction::ExportAccounts) => {
                    self.export_accounts();
                }
                Some(settings::SettingsAction::ImportAccounts) => {
                    self.import_accounts();
                }
                Some(settings::SettingsAction::LanguageChanged { language }) => {
                    self.config.language = language.as_str().to_string();
                    if let Err(e) = self.config.save(&self.config_path) {
                        tracing::warn!("Could not save language preference: {e}");
                    }
                    // Install immediately so the UI reflects the change on the
                    // next frame.
                    i18n::install(ctx, language);
                }
                None => {}
            }
        });
    ...
    fn show_confirm_remove_dialog(&mut self, ctx: &egui::Context) {
        ...
        let is_running = self.tracked_instances.iter().any(|i| i.user_id == uid);
        let lang = crate::i18n::of(ctx);
        let t = |key: &'static str| -> &'static str { crate::i18n::tr(lang, key) };

        let mut keep_open = true;
        egui::Window::new(t("Move account to trash?"))
            .resizable(false)
            .collapsible(false)
            .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
            .show(ctx, |ui| {
                if is_running {
                    ui.label(t("This account is running. Are you sure you want to move it to the trash?"));
                } else {
                    ui.label(format!("{} \"{label}\"? {}", t("Move account to trash?"), t("The account can be restored later.")));
                }
                ui.add_space(8.0);
                ui.horizontal(|ui| {
                    if ui
                        .button(format!("\u{1f5d1}  {}", t("Move to trash")))
                        .clicked()
                    {
                        self.move_to_trash(uid);
                        keep_open = false;
                    }
    ...
    /// Move an account from the main list to the trash, preserving all its
    /// data so it can be restored later.
    fn move_to_trash(&mut self, user_id: u64) {
        if let Some(pos) = self.store.accounts.iter().position(|a| a.user_id == user_id) {
            let account = self.store.accounts.remove(pos);
            self.store.trashed.push(account);
            self.selected_ids.remove(&user_id);
            self.auto_save();
            let lang = crate::i18n::Language::from_str(&self.config.language);
            self.toasts.push(Toast::info(format!(
                "{}",
                crate::i18n::tr(lang, "Moved {} account(s) to trash")
                    .replace("{}", "1")
            )));
        }
    }

    /// The trash panel — a floating window listing trashed accounts with
    ...
    fn show_trash_window(&mut self, ctx: &egui::Context) {
        ...
        let lang = crate::i18n::of(ctx);
        let t = |key: &'static str| -> &'static str {
            crate::i18n::tr(lang, key)
        };

        let mut action: Option<TrashAction> = None;

        egui::Window::new(format!("\u{1f5d1}  {}", t("Trash")))
            .id(egui::Id::new("trash_window"))
            .default_size([400.0, 350.0])
            .min_size([300.0, 200.0])
            .open(&mut self.trash_open)
            .show(ctx, |ui| {
                if self.store.trashed.is_empty() {
                    ui.vertical_centered(|ui| {
                        ui.add_space(30.0);
                        ui.label(egui::RichText::new("\u{1f5d1}").size(32.0).color(ui.visuals().weak_text_color()));
                        ui.add_space(8.0);
                        ui.label(t("No accounts in the trash."));
                    });
                    return;
                }
                ...
                // "Empty trash" button
                ui.horizontal(|ui| {
                    ui.label(format!("{}{}", self.store.trashed.len(), t("Trash")));
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.button(t("Empty trash")).clicked() {
                            action = Some(TrashAction::EmptyTrash);
                        }
                    });
                });
                ui.separator();
                ...
                egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
                    for (idx, account) in self.store.trashed.iter().enumerate() {
                        ...
                        card.show(ui, |ui: &mut egui::Ui| {
                            ui.horizontal(|ui| {
                                ui.vertical(|ui| {
                                    ui.strong(account.label());
                                    ui.label(
                                        egui::RichText::new(t("Recently removed"))
                                            .small()
                                            .color(ui.visuals().weak_text_color()),
                                    );
                                });
                                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                    if ui.button(t("Restore")).clicked() {
                                        action = Some(TrashAction::Restore(idx));
                                    }
                                    if ui.button(t("Delete permanently")).clicked() {
                                        action = Some(TrashAction::PermanentDelete(idx));
                                    }
                                });
                            });
                        });
        ...
        match action {
            Some(TrashAction::Restore(idx)) if idx < self.store.trashed.len() => {
                let account = self.store.trashed.remove(idx);
                self.store.accounts.push(account);
                self.auto_save();
                self.toasts.push(Toast::success(t("Restored account")));
            }
            Some(TrashAction::PermanentDelete(idx)) if idx < self.store.trashed.len() => {
                let uid = self.store.trashed[idx].user_id;
                // Confirm first
                self.confirm_permanent_delete = Some(idx);
            }
            Some(TrashAction::EmptyTrash) => {
        ...
        if let Some(idx) = self.confirm_permanent_delete {
            let mut confirm_open = true;
            egui::Window::new(t("Delete permanently"))
                .resizable(false)
                .collapsible(false)
                .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
                .show(ctx, |ui| {
                    ui.label(t("This action cannot be undone."));
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if ui.button(t("Delete permanently")).clicked() {
                            if idx < self.store.trashed.len() {
                                let uid = self.store.trashed[idx].user_id;
                                self.store.trashed.remove(idx);
                                // Clean credential manager
                                self.bridge.send(BackendCommand::RemoveAccount { user_id: uid });
                                self.auto_save();
                                self.toasts.push(Toast::info(t("Account permanently deleted")));
                            }
                            self.confirm_permanent_delete = None;
                            confirm_open = false;
                        }
        ...
        if self.confirm_empty_trash {
            let mut confirm_open = true;
            egui::Window::new(t("Empty trash"))
                .resizable(false)
                .collapsible(false)
                .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
                .show(ctx, |ui| {
                    ui.label(t("Delete all trashed accounts permanently?"));
                    ui.add_space(4.0);
                    ui.label(t("This action cannot be undone."));
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if ui.button(t("Empty trash")).clicked() {
                            for account in self.store.trashed.drain(..) {
                                self.bridge.send(BackendCommand::RemoveAccount { user_id: account.user_id });
                            }
                            self.auto_save();
                            self.toasts.push(Toast::info(t("Trash emptied")));
                            self.confirm_empty_trash = false;
                            confirm_open = false;
                        }
                        if ui.button(t("Cancel")).clicked() {
