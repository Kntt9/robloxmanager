...
pub enum PresetsAction {
    ...
    RevealFolder,
    /// Re-scan the presets folder for files added or removed while the app is
    /// open (e.g. the user dropped a preset file into it manually).
    Rescan,
}
        ...
        section_frame.show(ui, |ui: &mut egui::Ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui
                        .small_button("\u{1f504}  Reload")
                        .on_hover_text(
                            "Re-scan the presets folder for new or removed presets",
                        )
                        .clicked()
                    {
                        action = Some(PresetsAction::Rescan);
                    }
                    if ui
