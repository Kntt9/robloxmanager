...
pub fn show(
    ...
    ui.vertical(|ui| {
        // Header
        ui.horizontal(|ui| {
            ui.heading(t("{} Accounts Selected").replace("{}", &count.to_string()));
            if ui.small_button(t("Clear selection")).clicked() {
                ...
                for (idx, account) in selected_accounts.iter().enumerate() {
                    ui.horizontal(|ui| {
                        ...
                        ui.label(if anonymize {
                            t("Account {}").replace("{}", &(idx + 1).to_string())
                        } else {
        ...
        ui.heading(t("Bulk Launch"));
        ui.add_space(4.0);
        ui.label(t("All selected accounts will join the same server sequentially."));
        ui.add_space(4.0);
