...
pub fn show(
    ...
    // Number of accounts in the trash, shown on the trash button.
    trash_count: usize,
) -> SidebarResult {
        ...
        ui.horizontal(|ui| {
            ui.add(
                egui::TextEdit::singleline(&mut state.search_query)
                    .hint_text("\u{1f50d}  Search accounts\u{2026}")
                    .desired_width(f32::INFINITY),
            );
        });

        // Sort selector + Add button row
        ui.horizontal(|ui| {
            ...
            if prev_sort != SortOrder::Custom && state.sort_order == SortOrder::Custom {
                // Positions will be assigned by the handler
            }

            // Add account button (accent fill)
            let add_btn_resp = ui.add(
                egui::Button::new("\u{2795}  Add")
            ...
            // Presets button — star + text so it is clearly the presets
            // manager, not a generic favorite star.
            let presets_label = format!(
                "\u{2b50}  {}",
                crate::i18n::tr(ui.lang(), "Presets")
            );
            if ui
                .add(
                    egui::Button::new(presets_label)
                        .min_size(egui::vec2(0.0, 26.0)),
                )
                .on_hover_text("Manage launch presets")
                .clicked()
            {
                actions.push(SidebarAction::ShowPresets);
            }

            // Trash button — shows count if non-empty.
            let trash_label = if trash_count > 0 {
                format!("\u{1f5d1}  {}", crate::i18n::tr(ui.lang(), "Trash"))
            } else {
                format!("\u{1f5d1}  {}", crate::i18n::tr(ui.lang(), "Trash"))
            };
            if ui
                ...
                .clicked()
            {
                actions.push(SidebarAction::OpenTrash);
            }
        });
...
fn render_account_row(
    ...
    // Always show a second line (username) unless anonymize is on, so the row
    // reads as a proper account entry, not just a name.
    let has_subtitle = !anonymize;
    let row_height = 46.0;
    let avatar_size = 34.0;
    ...
    // Left accent bar for the selected row, so selection is obvious at a glance.
    if is_selected {
