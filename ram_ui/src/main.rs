...
fn main() {
    ...
    let _ = eframe::run_native(
        ...
        Box::new(move |cc| {
            ...
            i18n::install(&cc.egui_ctx, saved_lang);
            Ok(Box::new(app::AppState::new(config, config_path)))
        }),
    );
}

/// Register the Windows color-emoji font (Segoe UI Emoji) as a fallback on both
/// text families, so game titles and other text render every emoji instead of
/// falling back to the tiny built-in icon font that leaves some as tofu boxes.
///
/// egui's own `emoji-icon-font` only covers a small set; any emoji it lacks is
/// drawn as `\u{FFFD}`-style boxes. Windows ships `seguiemj.ttf`, which covers
/// essentially the whole emoji block, so appending it after the built-in fonts
/// makes the glyph lookup reach it for anything the built-ins miss. This is a
/// pure fallback: Latin text still comes from the primary font.
fn setup_fonts(ctx: &eframe::egui::Context) {
    let mut fonts = eframe::egui::FontDefinitions::default();

    // Windows ships this font on Windows 10/11. On other systems (or if it is
    // missing), the built-in icon font remains as-is and rendering just stays
    // at its current quality.
    const SEGOE_EMOJI: &str = "C:\\Windows\\Fonts\\seguiemj.ttf";
    if std::path::Path::new(SEGOE_EMOJI).is_file() {
        let name = "segoe-emoji";
        if let Ok(bytes) = std::fs::read(SEGOE_EMOJI) {
            fonts
                .font_data
                .insert(name.to_owned(), std::sync::Arc::new(eframe::egui::FontData::from_owned(bytes)));
            // Append to every text family as the last fallback.
            for family in [
